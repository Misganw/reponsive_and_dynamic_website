...
class A_certificate extends CI_Controller {
	...
	public function index()
	{
        $user_id = base64_decode($this->input->get('I'));
         $data['certificate_list'] = $this->Certificate_model->getAll_certificate_by_user_id($user_id);
        $this->load->view('admin_certificate',$data);
 ...
 public function get_certificate()
	...
	{
        $data1['member_list'] = $this->Certificate_model->get_member();
		$this->load->view('add_certificate',$data1);
	}
    public function save_certificate()
    {
        ...
        {    
            ...
            $config = array(
                ...
                'upload_path' => "./assets/certificate",
            ...
            if (!$this->upload->do_upload('p_file'))
            {
                ...
                // $this->session->set_flashdata('upload_error','The photo does not uploaded !!!');
            ...
            else
            {
                ...
                redirect(site_url().'a_certificate/insert_success');
            }
        }
    ...
    function search()
    {
        ...
        $data = $this->db->get("tbl_certificate")->result(); 
        echo json_encode($data);
    }
    public function search_certificate()
    {
        $title = $this->input->post('title', TRUE);
        $data['s_certificate']=$this->Certificate_model->search_certificate($title); 
        $this->load->view('admin_certificate',$data);
    }
    public function edit_certificate()
    {
         $s_id = base64_decode($this->input->get('I'));
         $data['member_list'] = $this->Certificate_model->get_member();
		 $data['certificate'] = $this->Certificate_model->certificate();
         $data['basic_info']=$this->Certificate_model->certificate_edit($s_id);
    ...
    public function update_certificate()
    {
        ...
        {
        // echo $this->upload->display_errors();
            $this->Certificate_model->update_certificate($data,$id);
             redirect(site_url().'a_certificate/update_success');
        }
        else
        {
                //now get the file uploaded data 
            ...
            redirect(site_url().'a_certificate/edit_certificate');
    ...
    {
         $s_id = base64_decode($this->input->get('I'));
         $data['certificate_detail']=$this->Certificate_model->certificate_detail($s_id); 

        $this->load->view('certificate_detail', $data);     
    }
    
    public function delete_certificate()
    {
        $s_id = base64_decode($this->input->get('I'));
         $data['basic_info']=$this->Certificate_model->delete_certificate($s_id);   
        $this->load->view('admin_certificate', $data);     
    }
